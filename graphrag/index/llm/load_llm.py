...
def _load_qwen_chat_llm(
	  on_error: ErrorHandlerFn,
    cache: LLMCache,
    config: dict[str, Any],
): 
    print("$$$$$$ _load_qwen_chat_llm")
...
def _load_qwen_embedding_llm(
	  on_error: ErrorHandlerFn,
    cache: LLMCache,
    config: dict[str, Any],
):
    # print("$$$$$$ config#6", config)
    return _create_qwen_embedding_llm(
			  OpenAIConfiguration({
            **_get_base_config(config),
            "model": config.get(
                "embeddings_model", config.get("model", "text-embedding-3-small")
...
loaders = {
    ...
    LLMType.QwenEmbedding: {
			  "load": _load_qwen_embedding_llm,
        "chat": False,
		},
    LLMType.QwenChat: {
			  "load": _load_qwen_chat_llm,
        "chat": True,
		}
}
...
def _create_qwen_embedding_llm(
		configuration: OpenAIConfiguration,
    on_error: ErrorHandlerFn,
    cache: LLMCache,
) -> EmbeddingLLM:
    """Create an qwen embeddings llm"""
    # print("$$$$$$ config#7", configuration)
    result = QwenEmbeddingsLLM(configuration)
